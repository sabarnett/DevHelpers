...
public class ColorPickerView: NSView {
    ...
    private var selectedHSB = NSColor.green.usingColorSpace(.sRGB)!.hsb
    ...
    @IBInspectable public var colorWheelBorderColor: NSColor? {
        didSet {
    ...
    @IBInspectable public var indicatorBorderColor: NSColor? = NSColor(white: 0.9, alpha: 0.8) {
        didSet {
    ...
    private lazy var indicatorLayer: CALayer = {
        ...
        indicatorLayer.shadowColor = NSColor.black.usingColorSpace(.sRGB)!.cgColor
    ...
    public func updateSelectedColor(_ color: NSColor) {
        guard let layer else { return }
...
//    public override func touchesBegan(_ touches: Set<NSTouch>, with event: NSEvent?) {
...
//    public override func touchesMoved(_ touches: Set<NSTouch>, with event: NSEvent?) {
...
//    public override func touchesEnded(_ touches: Set<NSTouch>, with event: NSEvent?) {
...
extension ColorPickerView {
    ...
    private func createHSColorWheelImage(size: CGSize) -> CGImage {
        ...
        let colorWheelDiameter = frame.width == 0 ? 200 : Int(frame.width)
        ...
        for yPos in 0 ..< colorWheelDiameter {
            for xPos in 0 ..< colorWheelDiameter {
                var hue: CGFloat = 0
                ...
                let offset = Int(4 * (xPos + yPos * colorWheelDiameter))
    ...
    private func getPointFromHS(hue: CGFloat, saturation: CGFloat) -> CGPoint {
        ...
        let xPos = colorWheelDiameter / 2 + radius * cos(hue * .pi * 2)
        let yPos = colorWheelDiameter / 2 + radius * sin(hue * .pi * 2)
